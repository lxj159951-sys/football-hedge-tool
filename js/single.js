...
const Single = {
    ...
    calculate() {
        // 获取输入值
        const match = document.getElementById('singleMatch').value.trim() || '单关比赛';
        const odds = parseFloat(document.getElementById('singleOdds').value);
        const stake = parseFloat(document.getElementById('singleStake').value);
        const cf = parseFloat(document.getElementById('singleCF').value) || 12;
        ...
        // 对冲计算
        const hedgeOdds = odds * 0.9; // 假设对冲赔率为原赔率的90%
    ...
    displayResult(result) {
        const resultCard = document.getElementById('singleResult');
        const resultBody = document.getElementById('singleResultBody');
        ...
        resultBody.innerHTML = `
            ...
            <div class="result-actions">
                <button class="btn btn-primary" onclick="Single.savePlan()">
                    <i class="fas fa-save"></i> 保存方案
                </button>
                <button class="btn btn-secondary" onclick="Single.reset()">
    ...
    reset() {
        document.getElementById('singleMatch').value = '';
        document.getElementById('singleOdds').value = '';
        document.getElementById('singleStake').value = '';
